...
namespace 画面
{
void InitDefault( Menu* メニュー , const char* デフォルトメッセージ)
{
	メニュー->SetNow();

	Menu::AddFront(new 暗幕(255));
	Menu::AddBack(new エフェクト(MSystem::背景[6], 400, 300));
	
	Menu::Add(new メッセージ枠( デフォルトメッセージ ));
}
...
class 機体選択 : public Menu
{
	...
	public:
		...
		void Push()
		{
			...
			{
				case 0:Director::AddScene( new TestStage() ); break;
	...
	void Init()
	{
		...
		Menu::AddBack(new 裏画面(Director::GetScene(0)) );
		Menu::AddBack(new 暗幕(128));
		//選択可能なオブジェクト
		Menu::Add(new 機体ボタン( 280, 137 , 1 , "れいむ"	));
		Menu::Add(new 機体ボタン( 280, 187 , 2 , "まりさ"	));
		Menu::Add(new 機体ボタン( 280, 237 , 3 , "れみりゃ" ));
		Menu::Add(new 選択枠(	  310, 287 , 180 , 40 , "  戻る"	 , funcB ));
	}
};
...
public:
	void Init()
	{
        ...
        for(int i=0;i<6;++i)
        {
            Menu::Add( new エフェクト(&MFrame::BMPフレーム[2],250, 67 + i * 50,106,46) );
            Menu::Add( new エフェクト(&MFont::ゴシック, 250+11, 67 + i * 50+11,Color::White,1,{"Stage ", i+1}) );
        }

		//選択可能なオブジェクト
        for( int i=0 ; i<6*4 ; ++i )
        {
            Menu::Add(new 仕事ボタン( i/4 , i%4 ) );            
        }

		Menu::Add(new 選択枠(310, 367 , 180 , 40 , "  戻る"	 , funcB, true));
	}
};
...
public:
    強化選択(int 武器ID )
    {
		...
		Menu::Add( new エフェクト(&MFrame::BMPフレーム[2],200,210,400,50));

        if(武器ID == -1)
        {
            //素材が不足
            SetItemCount(1,1);
    		Menu::Add( new エフェクト(&MFont::ゴシック,234,222,Color::White,1,"素材が不足しています"));
            Menu::Add(new 選択枠( 410, 307  , 180 , 40 , "はい" , [&]{isEnd = true;} ));
            return;
        }
		...
		Menu::Add( new エフェクト(&MFont::ゴシック,234,222,Color::White,1,"素材を消費して強化しますか？"));

		//選択可能なオブジェクト
		Menu::Add(new 選択枠(	     210, 307 , 180 , 40  , "はい"	 , 武器強化 ));
        Menu::Add(new 選択枠(	     410, 307 , 180 , 40  , "いいえ" , [&](){isEnd = true;} ));
    }
};
...
private:
	...
	public:
		...
		void Draw() const
		{
			...
			//Menu::Add(new エフェクト(&MFrame::BMPフレーム,340,85,270,285));
			if(isSelect)
			{              
...
public:
	void Init()
	{
		InitDefault( this , "改造する装備を選んで下さい");
		SetItemCount(3,6);

		auto funcA = [&]{ isEnd = true; };
		//選択すると左に現在性能と強化後性能、必要素材を表示
		Menu::AddBack(new エフェクト(&MFrame::BMPフレーム[2],340,85,270,285));

		//Body3種
		//Menu::Add(new エフェクト(&MFont::白絵, 40 , 70 ,Color::Blue,2,"Body"));
		Menu::Add(new 開発プラン(190,85, 0 ));
		Menu::Add(new 開発プラン(240,85, 1 ));
		Menu::Add(new 開発プラン(290,85, 2 ));

		//MainWepon武器6種
		//Menu::Add(new エフェクト(&MFont::白絵, 35 , 135 ,Color::Blue,2,"Front"));
		Menu::Add(new 開発プラン(190,150, 3 ));
		Menu::Add(new 開発プラン(240,150, 4 ));
		Menu::Add(new 開発プラン(290,150, 5 ));

		//SideWepon
		//Menu::Add(new エフェクト(&MFont::白絵, 40 , 200 ,Color::Red,2,"Side"));
		Menu::Add(new 開発プラン(190,215, 6));
		Menu::Add(new 開発プラン(240,215, 7));
		Menu::Add(new 開発プラン(290,215, 8));

		//Option
		//Menu::Add(new エフェクト(&MFont::白絵, 30 , 265 ,Color::Blue,2,"Option"));
		Menu::Add(new 開発プラン(190,280, 9));
		Menu::Add(new 開発プラン(240,280,10));
		Menu::Add(new 開発プラン(290,280,11));

		Menu::Add(new 開発プラン(190,330,12));
		Menu::Add(new 開発プラン(240,330,13));
		Menu::Add(new 開発プラン(290,330,14));

		//選択可能なオブジェクト
		Menu::Add(new 選択枠( 310, 380 , 180 , 40 , "  戻る"	 , funcA, true));
	}
};
...
public:
	void Init()
	{
		...
		for(int i=0;i<36;++i)
		{
			Menu::Add(new 素材アイコン( 220 + i%6 * 60 , 150 + i/6 * 30 , i ) );			
		}
		Menu::Add(new 選択枠(310, 370 , 180 , 40 , "  戻る", funcA, true));
...
public:
	...
	void Init()
	{
		InitDefault( this , "条件を満たすと、職歴を得ます");
		SetItemCount(3,6);
        ...
        for(int i=0;i<15;++i)
        {
		    Menu::Add(new 勲章枠( 150 +i %3 * 170 , 70+i/3*50 , i ));
        }

		Menu::Add(new 選択枠(310 , 320 , 180 , 40  , " 戻る"	 , funcA, true));
	}
};
...
public:
	...
	void Init()
	{
		InitDefault( this , "音量を設定します");
		SetItemCount(1,4);

		Menu::Add(new 音量枠(200, 170 , "ＢＧＭ"	, Config::BGM音量));
		Menu::Add(new 音量枠(200, 220 , "効果音"	, Config::効果音量));
		Menu::Add(new 音量枠(200, 270 , "ボイス"	, Config::ゆっくりボイス));

		Menu::Add(new 選択枠(310, 320  , 180 , 40 , "戻る"	, [&]{ isEnd = true; } , true));
	}
};
...
public:
	void Init()
	{
		InitDefault( this , "ゆっくりしていってね！！！");
		SetItemCount(1,5);

		isCancel = false;

		auto funcA = [] { Menu::Add(new 暗幕(255)); Director::AddScene(new 画面::仕事()); };
		auto funcB = [] { Menu::Add(new 暗幕(255)); Director::AddScene(new 画面::武器屋());};
		auto funcC = [] { Menu::Add(new 暗幕(255)); Director::AddScene(new 画面::倉庫());};
		auto funcD = [] { Menu::Add(new 暗幕(255)); Director::AddScene(new 画面::戦績());};
		auto funcE = [] { Menu::Add(new 暗幕(255)); Director::AddScene(new 画面::システム());};
	
		//選択可能なオブジェクト
		Menu::Add(new 選択枠(310, 120 , 180 , 40  , " ハロワ ", funcA , true));
		Menu::Add(new 選択枠(310, 170 , 180 , 40  , " 武器屋 ", funcB , true));
		Menu::Add(new 選択枠(310, 220 , 180 , 40  , "  倉庫　", funcC , true));
		Menu::Add(new 選択枠(310, 270 , 180 , 40  , "  職歴  ", funcD , true));
		Menu::Add(new 選択枠(310, 320 , 180 , 40  , "　設定　", funcE , true));
	}
};
