...
namespace SDX_STG
{
    ...
    public:
        ...
        void Push()
        {
            if( !押下時の処理 || !SMenu->IsActive() ) return;

            if (暗転フラグ)
            {
                SMenu->AddFront(new 暗幕(0, Color::Black, 押下時の処理));
            }else{
                 押下時の処理();
            }
        }
    ...
    public:
        ...
        {
            SMenu->GetMessage() = 描画する文字列;
            auto 表示文字 = new SpFont(&MFont::ゴシック, Color::Black, 1, 1, SMenu->GetMessage() );
            表示文字->MoveGap(20, 32);
            Add(表示文字);
        }
    };
