...
public:
    ...
    void Dead()
    {
        ...
        //爆発
        MSound::爆発.Play();
        SStage->Add(new エフェクト(&MEffect::爆発[13], GetX(), GetY()));
        
        for(int i=0; i < 加算スコア / 1000 ;++i)
        {
            SStage->Add(new Item(GetX(), GetY(), 性能.レア率, 性能.ボスフラグ));
        }
    ...
    void Shoot(Image* 画像 , ISpeed* 速度 , double 角度, double 攻撃力 ,int 半径 ,int Xずれ = 0,int Yずれ = 0)
    {
        ...
        SStage->Add(new Shot(new Circle(GetX() + Xずれ, GetY() + Yずれ, 半径), new SpImage(画像), 速度, 角度, 攻撃力, false, Belong::Etc));
    }
    void Shoot(Anime* 画像, ISpeed* 速度, double 角度, int 攻撃力, int 半径, int Xずれ = 0, int Yずれ = 0)
    {
        攻撃力 *= 1 + StgSystem::難易度;

        SStage->Add(new Shot(new Circle(GetX() + Xずれ, GetY() + Yずれ, 半径), new SpAnime(画像), 速度, 角度, 攻撃力, false, Belong::Etc));
    }
